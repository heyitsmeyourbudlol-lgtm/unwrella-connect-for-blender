#!/usr/bin/env python3
"""N07 hallucination_block_fill — practice distill / eval / serve (N01 stamp).

OVERSEER_NICHE_PRACTICE_N07_2026_09_07

Same recipe as N01: schema → train/heldout → niche accuracy → checkpoint → serve.
Deterministic rule baseline + ckpt under ``notes/niche_distill/practice_n07/``.
Does **not** rewrite/prune train or heldout JSONL (assignment: no corpus prune).

Usage::

    python3 scripts/niche_n07_practice.py              # eval existing heldout + write ckpt
    python3 scripts/niche_n07_practice.py --eval-only
    python3 scripts/niche_n07_practice.py --serve 'Classify … as hallucination block fill_yes…'
"""
...
# OVERSEER_NICHE_PRACTICE_N07_2026_09_07
NEEDLE = "OVERSEER_NICHE_PRACTICE_N07_2026_09_07"
ROOT = Path(__file__).resolve().parents[1]
DISTILL = ROOT / "notes" / "niche_distill"
TRAIN_JSONL = DISTILL / "N07_hallucination_block_fill.jsonl"
HELDOUT_JSONL = DISTILL / "N07_hallucination_block_fill_heldout.jsonl"
PRACTICE_DIR = DISTILL / "practice_n07"
CKPT_JSON = PRACTICE_DIR / "checkpoint.json"
WEIGHTS_DIR = PRACTICE_DIR / "weights"
WEIGHTS_PT = WEIGHTS_DIR / "n07_hallucination_block_fill_practice.pt"

# Longest / most specific label phrases first (substring gold in corpus).
_LABEL_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
    (
        "hallucination block fill_escalate",
        re.compile(r"hallucination block fill_escalate", re.I),
    ),
    (
        "hallucination block fill_maybe",
        re.compile(r"hallucination block fill_maybe", re.I),
    ),
    (
        "hallucination block fill_no",
        re.compile(r"hallucination block fill_no", re.I),
    ),
    (
        "hallucination block fill_yes",
        re.compile(r"hallucination block fill_yes", re.I),
    ),
    ("abstain", re.compile(r"\babstain\b", re.I)),
...
def predict(item: str) -> dict[str, str]:
    """Rule baseline for N07 — first matching strategy-ritual label in text."""
    text = (item or "").strip()
...
def write_checkpoint(*, n_train: int, n_heldout: int, accuracy: float) -> None:
    ...
    rules = {
        "kind": "n07_rule_baseline",
        "version": _PT_VERSION,
        "needle": NEEDLE,
        "predict": "scripts/niche_n07_practice.py:predict",
    ...
    manifest = {
        "needle": NEEDLE,
        "niche_id": "N07",
        "niche": "hallucination_block_fill",
        ...
        "serve": "python3 scripts/niche_n07_practice.py --serve '<item>'",
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--serve", metavar="ITEM", help="label strategy ritual → JSON")
