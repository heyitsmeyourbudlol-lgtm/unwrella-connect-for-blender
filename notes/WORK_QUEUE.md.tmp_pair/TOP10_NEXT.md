...
|-----:|----|-------|-----|------------|--------|
| 1 | T10-04 | Non-noop cycles ≥8 / day | Factory throughput gap (scoreboard) | Scoreboard week ≥8 non-noop cycles/day; CLEAN peer+improve up; Active nonempty | open |
...
- **Measured (scoreboard 04:22Z):** peer+improve **active**; Active nonempty; **7 today · proj 38.5/day · week_avg 7.0 [GAP]** — `meets_bar=false` (need today≥8 observed; proj alone cannot PASS). Do **not** false-close. Scoreboard meters CLEAN only (Mac sidecar no longer inflates).
...
- **Residual:** still below ≥8 observed/day; deferred still dominates many ticks (unittest/self-check races under swarm) — prefer agent non-noop over local_only; leave Active fuel; promote only after PASS.
- **Promote gate:** Queue line **TOP10_NEXT promote next after T10-04** stays **open** until this row is `done` + `./scripts/peer top10-refresh`. No episodic→Hot seed until a slot frees after refresh.
