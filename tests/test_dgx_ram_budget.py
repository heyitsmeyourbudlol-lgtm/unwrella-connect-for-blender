...
class TestDgxRamBudget(unittest.TestCase):
    ...
    def test_unittest_cwd_in_hub_excludes_foreign(self) -> None:
        """OVERSEER_UNITTEST_CENSUS_HUB_CWD_2026_09_07 — Doc2Api ≠ hub census."""
        hub = Path("/home/arnavrastogi/Automation")
        with mock.patch.object(
            budget, "_proc_cwd", return_value=Path("/home/arnavrastogi/Doc2Api")
        ):
            self.assertFalse(budget._unittest_cwd_in_hub(207768, hub=hub))
        with mock.patch.object(
            budget, "_proc_cwd", return_value=hub / ".worktrees" / "peer-0"
        ):
            self.assertTrue(budget._unittest_cwd_in_hub(1, hub=hub))
        with mock.patch.object(budget, "_proc_cwd", return_value=None):
            self.assertTrue(budget._unittest_cwd_in_hub(1, hub=hub))

    def test_list_unittest_worker_pids_hub_scoped(self) -> None:
        hub = Path("/home/arnavrastogi/Automation")
        rows = [
            (111, "python3 -m unittest discover -s tests -q"),
            (222, "python3 -m unittest discover -s tests -q"),
        ]

        def _cwd(pid: int) -> Path | None:
            if pid == 111:
                return Path("/home/arnavrastogi/Doc2Api")
            return hub / ".worktrees" / "peer-3"

        with mock.patch.object(budget, "iter_proc_cmdlines", return_value=rows), mock.patch.object(
            budget, "_proc_cwd", side_effect=_cwd
        ), mock.patch.object(budget.auto, "ROOT", hub):
            self.assertEqual(budget.list_unittest_worker_pids(), [222])
            self.assertEqual(budget.unittest_worker_count(), 1)
